...
function Monitor(props) {
  ...
  // list permission
  const isCallMonitor = useMemo(
    () => checkPermission(user.currentUser.permissions, CALL_CENTER_MANAGEMENT.callMonitor),
    [user.currentUser.permissions],
  );
  const isMonitorQueue = useMemo(
    () => checkPermission(user.currentUser.permissions, CALL_CENTER_MANAGEMENT.monitorQueue),
    [user.currentUser.permissions],
  );
  const isMonitorAgent = useMemo(
    () => checkPermission(user.currentUser.permissions, CALL_CENTER_MANAGEMENT.monitorAgent),
    [user.currentUser.permissions],
  );

  if (/* (!isCallMonitor && !isMonitorQueue && !isMonitorAgent) ||  */ false) {
