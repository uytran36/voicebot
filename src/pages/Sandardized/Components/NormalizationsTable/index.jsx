...
RenderNormalizationsTable.propTypes = {
...
function RenderNormalizationsTable({
  dispatch,
  ...
  const handleDeleteRecord = useCallback(
    (record) => async () => {
      const res = await functions.handleDeleteOmniCOntactListNormalization(headers, record.id);
      if (res && res.status) {
      ...
      render: (_, record) => {
        return (
          <div>
            ...
            />
            <Popconfirm
              title={`Bạn có muốn xóa #${record.tentailieu}`}
  ...
  return (
    <Table
      ...
      request={async ({ search, pageSize, current }) => {
        ...
        if (search.length > 0) {
          params.where = {
            'xlsContactObject.tentailieu': {
              regexp: `.*${search}*.`
            },
          };
        }
        const { data, total } = await functions.fetchContactNormalizations(headers, params);
...
export default connect(({ user }) => ({ user }))(RenderNormalizationsTable);
