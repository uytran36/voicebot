...
import { requestImportUser } from '@/services/crm';
...
function ImportUser({ dispatch, onToggleBack, userId, authToken, tokenGateway, wsId }) {
  ...
  const handleAcceptReplaceFile = useCallback(
    (fileReplace, subId, action = 0) => async () => {
      ...
      try {
        const res = await requestImportUser(formData, headers);
        if (res.status) {
          setStateModal(false, {});
          hide();
                ...
                extra={
                  <Space>
                    ...
                    <Button
                      type="primary"
                      onClick={() =>
                        window.open(`${api.UMI_API_BASE_URL}/api/files/${url}`).focus()
                      }
                    >
      ...
      <Row justify="center" align="middle" style={{ marginTop: '66px' }}>
        <Col>
          <div className={styles['upload-container']}>
            <strong style={{ marginRight: '20px' }}>Tải lên file Excel:</strong>
