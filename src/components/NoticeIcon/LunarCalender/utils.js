...
const Utils = (function(){
    ...
    function LunarDate(dd, mm, yy, leap, jd, llen) {
        this.day = dd;
        this.month = mm;
        this.year = yy;
        this.leap = leap;
    ...
    function decodeLunarYear(yy, k) {
        ...
        // console.log(333, regularMonths, ly)
        return ly;
    }
    ...
    function findLunarDate(jd, ly) {
        if (jd > LAST_DAY || jd < FIRST_DAY || ly[0].jd > jd) {
            return new LunarDate(0, 0, 0, 0, jd, 30);
        }
        var i = ly.length-1;
        while (jd < ly[i].jd) {
            i--;
        ...
        var off = jd - ly[i].jd;
        var ret = new LunarDate(ly[i].day+off, ly[i].month, ly[i].year, ly[i].leap, jd, ly[i].llen);
        return ret;
    }
    ...
    */
    ...
    //showDayInfo        (16,13,7,2019,0,29,2458709, 13,8,2019);"
    //function alertDayInfo   (dd, mm, yy, leap, jd, sday, smonth, syear) {
    function showDayInfo(cell, dd, mm, yy, leap, length, jd, sday, smonth, syear) {
        selectCell(cell);
        ...
        return {
            sday, dayOfWeek, dd, tenthang, namam:'N\u0103m '+getYearCanChi(yy), 
            canchithang:'Th\u00E1ng '+thang,
            canchingay: 'Ng\u00E0y '+ngay,
            canchigio: 'Gi\u1EDD '+getCanHour0(jd)+' '+CHI[0],
            tietkhi: 'Ti\u1EBFt '+TIETKHI[getSolarTerm(jd+1, 7.0)],
            dayinfo: getDayInfo(dd, mm),
            hoangdao: 'Gi\u1EDD ho\u00E0ng \u0111\u1EA1o: '+getGioHoangDao(jd)
        }
    }

    function getDateInfo(date=null){
        var {sday,smonth,syear} = date || {sday:today.getDate(), smonth:today.getMonth()+1, syear:today.getFullYear()}
        var lunar = getLunarDate(sday, smonth, syear)
        // console.log(8888,sday, smonth, syear)
        var {day:dd, month:mm, year:yy, leap, llen, jd} = lunar
        var dayOfWeek = TUAN[(jd + 1) % 7]
        var nhuan = (leap == 1) ? ' nhuận' : '';
        var tenthang = 'Th\u00E1ng '+THANG[mm-1]+nhuan+(llen == 30 ? ' (\u0110)' : ' (T)');
        var loaithang = nhuan+(llen == 30 ? ' (đủ)' : ' (thiếu)');
        var thang = CAN[(yy*12+mm+3) % 10] + " " + CHI[(mm+1)%12];
        var ngay = CAN[(jd + 9) % 10] + " " + CHI[(jd+1)%12];
        return {
            sday, smonth,syear, dayOfWeek, dd, mm, yy, leap, tenthang, loaithang, namam:'N\u0103m '+getYearCanChi(yy), 
            canchithang:'Th\u00E1ng '+thang,
            canchingay: 'Ng\u00E0y '+ngay,
            canchigio: 'Gi\u1EDD '+getCanHour0(jd)+' '+CHI[0],
            tietkhi: 'Ti\u1EBFt '+TIETKHI[getSolarTerm(jd+1, 7.0)],
    ...
    //     // window.window.setTimeout("Utils.showVietCal()",5000);
    // }
    ...
    */
    ...
    function formatYYMMDD(dateString){
        var dateFormat = new Date(dateString);
        return [dateFormat.getFullYear(),dateFormat.getMonth() <10 ? '0'+(dateFormat.getMonth()+1):dateFormat.getMonth()+1,dateFormat.getDate() <10 ? '0'+dateFormat.getDate():dateFormat.getDate()].join('-') 
    }
